---
title: Six Things I Did Not Know About Scala And Its Standard Library
published: false
...
I am working with Scala as my main programming language for about three years now. I recently went through some basic [Scala language exercises](https://www.scala-exercises.org/std_lib) for fun and discovered some things I did not know about.

Some of the things I find really useful. Others should not have made it into the language in my opinion. In this blog posts we will talk about
...
In functional programming, you treat your program as a mathematical function, being composed out of smaller functions. Functions are pure, side-effect free transformations from input to output.
...
- If you cannot fix your domain, you can try to fix your partial function. Instead defining division as `(Int, Int) -> Int`, define it as `(Int, Int) -> Option[Int]` and return `None` in case the divisor is 0. Now you no longer have a partial function. In case of `head` you can use `headOption` instead.
...
- An extractor can be any stable object, including instantiated classes with an unapply method (https://www.scala-exercises.org/std_lib/extractors)

## Conclusion

I think that even if you consider yourself to be a senior, expert, guru, rock star or whatever, there are always things you do not know and it never hurts to revisit the basics from time to time.
